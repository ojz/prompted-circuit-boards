...
| **Copperhead** (Apache-2.0 CLI, TypeScript, Show HN 2026-09-08, 111 stars) | `schematic.intent.json`: parts, nets as `REF.PIN` lists, no-connects, group hints; no coordinates. A deterministic drafting engine computes the schematic: 1.27 mm grid, subsystems left-to-right, longest-path layering into columns, barycenter ordering, local wires only for ≤4-pin in-group nets, everything else matched net labels, power symbols never wired. UUIDv5 from semantic paths, library symbols vendored, byte-identical output. | **Not an autorouter** by its own statement. Produces a placed, DRC-clean draft; routing "stays human or delegated". Zones are KiCad-native. | `kicad-cli` ERC/DRC as gates; exports Gerber/STEP/DXF/BOM. Cloud tier $49/user/month, free CLI. | LLM agent (Claude, GPT, Codex, Ollama) writes intent + markdown docs, engine drafts, gates block commits that break documented budgets. The schematic-drafting design and its 13 legibility rule families are the part worth borrowing for `pcbgen`. Same architecture as this repo's toolkit (intent → deterministic emit → kicad-cli gates), arrived at independently. Example board (Open Telegraph, ESP32-S3) not yet fabricated as of the HN post. |
