...
The KiCad project in this directory is generated by `toolkit/src/Designs/Mult.hs`
(`cd toolkit && cabal run pcbgen -- mult`) and verified by `toolkit/check.sh mult`.
Do not edit the `.kicad_*` files by hand.

- Schematic: ERC clean.
- Layout: DRC clean with schematic parity, zero unconnected items.
